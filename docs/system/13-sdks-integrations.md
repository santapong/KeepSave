...
> **Implementation state (be precise):** in code today are the *scrubbed-env*, *metacharacter reject*, *`exec.CommandContext` + timeout* (part B) **and a binary allowlist** — `validateMCPEntryCommand` restricts the interpreter to `node`/`python`/`python3` and rejects path separators (`handlers_mcp_gateway.go`). Still **not wired** from [ADR-0010](../adr/0010-mcp-gateway-command-execution-hardening.md): the `go-binary` entry and *true argv-array* input (the command is still split with `strings.Fields` rather than passed as an array), the **`safego` harness** (part C — no `backend/internal/runtime/` package exists), and the **build budget** (part D). Treat the MCP exec surface as mostly-but-not-fully hardened. See [operations](./10-operations.md) §3.
