...
Space Grotesk is the UI/display face; JetBrains Mono is used for code and compact labels. Cards use 14–20px corners, quiet borders, and restrained shadows. Forms, menus, dialogs, toasts, tables, and charts inherit shared semantic tokens. Primary actions use lavender; success, warning, danger, and Alpha/UAT/PROD keep their semantic distinctions. Light mode is retained as a violet daylight variant. Landing remains the dark brand presentation.
