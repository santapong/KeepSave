...
func SetupRouter(
	...
	v1 := r.Group("/api/v1")
	{
		...
		ak := v1.Group("/api-keys")
		ak.Use(JWTAuthMiddleware(jwtService))
		{
		...
		og.Use(JWTAuthMiddleware(jwtService))
		{
			og.POST("", orgHandler.Create)
			og.GET("", orgHandler.List)
			og.GET("/:orgId", orgHandler.Get)
			og.PUT("/:orgId", orgHandler.Update)
			og.DELETE("/:orgId", orgHandler.Delete)
			og.POST("/:orgId/members", orgHandler.AddMember)
			og.GET("/:orgId/members", orgHandler.ListMembers)
			og.PUT("/:orgId/members/:userId", orgHandler.UpdateMemberRole)
		...
		tpl.Use(JWTAuthMiddleware(jwtService))
		{
			tpl.POST("", templateHandler.Create)
			tpl.GET("", templateHandler.List)
			tpl.GET("/builtin", templateHandler.ListBuiltin)
			tpl.GET("/:templateId", templateHandler.Get)
			tpl.PUT("/:templateId", templateHandler.Update)
			tpl.DELETE("/:templateId", templateHandler.Delete)
		...
		ag := v1.Group("/agent")
		ag.Use(APIKeyAuthMiddleware(jwtService, apikeyRepo))
		{
		...
		app.Use(APIKeyAuthMiddleware(jwtService, apikeyRepo))
		{
			app.POST("", applicationHandler.Create)
			app.GET("", applicationHandler.List)
			app.GET("/:appId", applicationHandler.Get)
			app.PUT("/:appId", applicationHandler.Update)
			app.DELETE("/:appId", applicationHandler.Delete)
			app.POST("/:appId/favorite", applicationHandler.ToggleFavorite)
		}
	}
