...
func NewRecommendationService(db *sql.DB, dialect repository.Dialect, secretRepo *repository.SecretRepository, projectRepo *repository.ProjectRepository, envRepo *repository.EnvironmentRepository, cryptoSvc *crypto.Service, aiMgr *AIProviderManager) *RecommendationService {
...
func (s *RecommendationService) GenerateRecommendations(projectID, userID uuid.UUID) ([]models.SecretRecommendation, error) {
	...
	if s.aiMgr != nil && s.aiMgr.HasProvider() {
		...
		resp, _, _, err := s.aiMgr.Chat("You are a security expert analyzing secret configurations. Return ONLY a JSON array. Never include actual secret values.",
		...
		if err == nil {
			...
			if json.Unmarshal([]byte(extractJSON(resp)), &aiRecs) == nil {
				for _, ar := range aiRecs {
					recommendations = append(recommendations, models.SecretRecommendation{
						ID: uuid.New(), ProjectID: projectID, RecommType: ar.Type, Severity: ar.Severity,
						Title: ar.Title, Description: ar.Description, AffectedKeys: ar.AffectedKeys,
						SuggestedAction: ar.SuggestedAction, Status: "pending", CreatedAt: time.Now(),
					})
				}
			}
		}
