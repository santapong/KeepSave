...
func (s *OAuthService) Authorize(clientID string, userID uuid.UUID, redirectURI string, scopes []string, codeChallenge, codeChallengeMethod string) (string, error) {
	...
	authCode := &models.OAuthAuthorizationCode{
		...
		Scopes: scopes, CodeChallenge: codeChallenge, CodeChallengeMethod: codeChallengeMethod,
...
func (s *OAuthService) issueTokens(clientID uuid.UUID, userID *uuid.UUID, scopes []string) (*models.OAuthTokenResponse, error) {
	...
	token := &models.OAuthToken{
		AccessTokenHash: hashSecret(rawAccessToken), RefreshTokenHash: hashSecret(rawRefreshToken),
		ClientID: clientID, UserID: userID, Scopes: scopes, TokenType: "Bearer",
		ExpiresAt: accessExpiry, RefreshExpiresAt: &refreshExpiry,
	}
	if err := s.oauthRepo.CreateToken(token); err != nil {
		return nil, fmt.Errorf("creating token: %w", err)
	}
	return &models.OAuthTokenResponse{AccessToken: rawAccessToken, TokenType: "Bearer", ExpiresIn: 3600, RefreshToken: rawRefreshToken, Scope: strings.Join(scopes, " ")}, nil
}
