...
func TestSSOService_DeleteEmitsAudit(t *testing.T) {
	...
	// DeleteSSOConfig requires org admin (AUTH-03), so seed a members table.
	if _, err := db.Exec(`CREATE TABLE organization_members (
		...
		UNIQUE (organization_id, user_id))`); err != nil {
	...
	orgID := uuid.New()
	actor := uuid.New()
	if _, err := db.Exec(`INSERT INTO organization_members (id, organization_id, user_id, role) VALUES (?,?,?,'admin')`,
		uuid.New().String(), orgID.String(), actor.String()); err != nil {
